...
import { userUploadImage } from '../core/operatedApi.js'
...
export function ProfileView({ persona, onSave, userToken }) {
  ...
  const addInterest = () => {
    const normalized = String(interestInput || '').trim()
    if (!normalized) return
    if (interests.some((item) => item.toLowerCase() === normalized.toLowerCase())) {
      setInterestInput('')
      return
    }
    set('interests', [...interests, normalized].slice(0, 6))
    setInterestInput('')
  }
  const prompts = useMemo(() => {
    const list = Array.isArray(draft.prompts) ? draft.prompts : []
    const byPosition = new Map(list.map((item) => [Number(item?.position || 0), item]))
    return [1, 2, 3].map((position) => {
      const existing = byPosition.get(position)
      if (existing) return existing
      return {
        position,
        title: position === 1 ? 'A PERFECT SUNDAY' : position === 2 ? 'NON-NEGOTIABLES' : 'CONVERSATION STARTER',
        answer: '',
      }
    })
  }, [draft.prompts])
  const setPrompt = (position, key, value) => {
    setDraft((prev) => {
      const base = Array.isArray(prev.prompts) ? prev.prompts : []
      const next = [1, 2, 3].map((index) => {
        const found = base.find((item) => Number(item?.position) === index)
        const prompt = found || (index === 1
          ? { position: 1, title: 'A PERFECT SUNDAY', answer: '' }
          : index === 2
            ? { position: 2, title: 'NON-NEGOTIABLES', answer: '' }
            : { position: 3, title: 'CONVERSATION STARTER', answer: '' })
        if (index !== position) return prompt
      ...
      <div className="space-y-4">
        <section className={`rounded-lg border ${op.borderSoft} ${op.bgElev} p-4`}>
          ...
          <div className="mt-4 grid grid-cols-2 gap-3">
            ...
            <Field label="Pronouns"><input className={inputClass} value={draft.pronouns || ''} onChange={(event) => set('pronouns', event.target.value)} /></Field>
            <Field label="Looking for"><input className={inputClass} value={draft.looking || ''} onChange={(event) => set('looking', event.target.value)} /></Field>
            <Field label="City"><input className={inputClass} value={draft.city || ''} onChange={(event) => set('city', event.target.value)} /></Field>
            <Field label="Country"><input className={inputClass} value={draft.country || ''} onChange={(event) => set('country', event.target.value.toUpperCase())} /></Field>
            <Field label="Latitude"><input className={inputClass} value={draft.latApprox || ''} onChange={(event) => set('latApprox', event.target.value)} /></Field>
            <Field label="Longitude"><input className={inputClass} value={draft.lngApprox || ''} onChange={(event) => set('lngApprox', event.target.value)} /></Field>
        ...
        <section className={`rounded-lg border ${op.borderSoft} ${op.bgElev} p-4`}>
          ...
          <div className="mt-4 space-y-3">
            <Field label="Bio">
              <textarea className={`${inputClass} min-h-24 resize-none`} value={draft.bio || ''} maxLength={500} onChange={(event) => set('bio', event.target.value)} />
            </Field>
            <Field label="About me (legacy)">
              <textarea className={`${inputClass} min-h-20 resize-none`} value={draft.aboutMe || ''} maxLength={500} onChange={(event) => set('aboutMe', event.target.value)} />
            </Field>
            ...
            <div className="grid grid-cols-1 gap-3">
              {prompts.map((prompt) => (
                <div key={prompt.position} className={`rounded-md border ${op.borderSoft} ${op.bgMain} p-3`}>
                  <div className="grid grid-cols-2 gap-3">
                    <Field label={`Prompt ${prompt.position} title`}>
                      <input
                        className={inputClass}
                        value={prompt.title || ''}
                        maxLength={80}
                        onChange={(event) => setPrompt(prompt.position, 'title', event.target.value)}
                      />
                    </Field>
                    <Field label={`Prompt ${prompt.position} answer`}>
                      <input
                        className={inputClass}
                        value={prompt.answer || ''}
                        maxLength={160}
                        onChange={(event) => setPrompt(prompt.position, 'answer', event.target.value)}
                      />
                    </Field>
                  </div>
                </div>
            ...
            <div className="flex gap-2">
              ...
              />
              <Button type="button" onClick={addInterest} disabled={!String(interestInput || '').trim()}>
        ...
        <section className={`rounded-lg border ${op.borderSoft} ${op.bgElev} p-4`}>
          ...
          <div className="mt-4 grid grid-cols-2 gap-3">
            <Field label="Relationship status"><select className={inputClass} value={draft.relStatus || 'single'} onChange={(event) => set('relStatus', event.target.value)}><option value="single">Single</option><option value="in_relationship">In relationship</option><option value="married">Married</option><option value="non_monogamous">Non-monogamous</option><option value="complicated">Complicated</option><option value="prefer_not_say">Private</option></select></Field>
            <Field label="Intent"><select className={inputClass} value={draft.intent || 'serious'} onChange={(event) => set('intent', event.target.value)}><option value="serious">Long-term</option><option value="dating">Dating</option><option value="figuring_out">Figuring it out</option><option value="casual">Casual</option><option value="friends">Friends</option><option value="non_monogamy">Non-monogamy</option></select></Field>
            ...
            <Field label="Kids"><input className={inputClass} value={draft.kids || ''} onChange={(event) => set('kids', event.target.value)} /></Field>
            <Field label="Pets"><input className={inputClass} value={draft.pets || ''} onChange={(event) => set('pets', event.target.value)} /></Field>
            <Field label="Diet"><input className={inputClass} value={draft.diet || ''} onChange={(event) => set('diet', event.target.value)} /></Field>
            <Field label="Exercise"><input className={inputClass} value={draft.exercise || ''} onChange={(event) => set('exercise', event.target.value)} /></Field>
            <Field label="Religion"><input className={inputClass} value={draft.religion || ''} onChange={(event) => set('religion', event.target.value)} /></Field>
            <Field label="Zodiac"><input className={inputClass} value={draft.zodiac || ''} onChange={(event) => set('zodiac', event.target.value)} /></Field>
          </div>
        </section>
      ...
      <aside className="space-y-4">
        <section className={`rounded-lg border ${op.borderSoft} ${op.bgElev} p-4`}>
          ...
          <div className={`mt-3 overflow-hidden rounded-[28px] border ${op.border} ${op.bgMain} p-2`}>
            ...
            <div className={`space-y-3 p-3 text-sm ${op.dim}`}>
              <p>{draft.bio || draft.aboutMe || 'No bio yet.'}</p>
              <div className={`text-xs ${op.mute}`}>
                {[draft.pronouns, draft.looking].filter(Boolean).join(' · ') || 'Pronouns and looking preference unset.'}
              </div>
              <div className="flex flex-wrap gap-1.5">{interests.slice(0, 6).map((interest) => <Chip key={interest}>{interest}</Chip>)}</div>
              {!!prompts.length && (
                <div className="space-y-1 text-xs">
                  {prompts
                    .filter((prompt) => String(prompt.answer || '').trim())
                    .map((prompt) => (
                      <div key={prompt.position} className={`${op.text}`}>
                        <span className={`${op.mute}`}>{prompt.title || `Prompt ${prompt.position}`}:</span> {prompt.answer}
                      </div>
                    ))}
                </div>
              )}
              <div className="flex flex-wrap gap-1.5">
                {[draft.height, draft.drinks, draft.smokes, draft.kids, draft.pets, draft.diet, draft.exercise, draft.religion, draft.edu, draft.zodiac]
                  .filter(Boolean)
                  .slice(0, 10)
                  .map((item, index) => <Chip key={`${item}-${index}`}>{item}</Chip>)}
              </div>
            </div>
          </div>
        </section>
        <section className={`rounded-lg border ${op.borderSoft} ${op.bgElev} p-4`}>
          ...
          <div className={`mt-3 space-y-2 text-sm ${op.dim}`}>
            ...
            <div className="flex justify-between"><span>Verified badge</span><Chip tone={draft.verified ? 'ok' : 'warn'}>{draft.verified ? 'on' : 'off'}</Chip></div>
